...
def __stop__( context ):
    if( context.message.author.id == "131579027659030528" ):
        ...
        yield from context.channel.send( "Uh uh uh. You're not allowed to do that." )
...
@asyncio.coroutine
def hey( context ):
    yield from context.channel.send( "hey" )

@client.command( pass_context = True, description = "Provides a link to the repository for kenny_bot." )
@asyncio.coroutine
def repo( *args ):
    yield from context.channel.send( "https://github.com/cschwirian/kenny_bot" )
...
def improve( context, *, message ):
    if( str( message ).lower() == "kenny"):
        yield from context.channel.send( "Cannot be improved." )
    else:
        message_str = str( message ).replace( "b", ":b:" ).replace( "B", ":b:" )
        yield from context.channel.send( message_str )
...
def pic( context ):
    ...
    yield from context.channel.send( file=File(open("./images/pics/" + pic, "rb")) )
...
def kennypic( context ):
    ...
    yield from context.channel.send( file=File(open("./images/kenny/" + pic, "rb")) )

# TODO: Kenny quote of the day
@client.command()
@asyncio.coroutine
def quote():
    yield from context.channel.send( "I've been getting Yasuo in ARAM a lot recently." )

@client.command( pass_context = True )
@asyncio.coroutine
def chill_gif( context ):
    gif = random.choice( os.listdir( "./gifs/chill" ) )
    yield from context.channel.send( file=File(open("./gifs/chill/" + gif, "rb")) )
...
def roll( context, *, message ):
    try:
        ...
        if( num_dice > 64 ):
            yield from context.channel.send( "I can't hold all those dice there, friend." )
        elif ( num_sides > 1000000 ):
            yield from context.channel.send( "That's an unnecessarily large number, friend." )
        else:
            total_roll = 0
            for die in range( num_dice ):
                total_roll += random.randint( 1, num_sides )

            yield from context.channel.send( "You rolled a %d." % total_roll )

    except:
        yield from context.channel.send( "You messed up the roll somehow. None of my business." )

@client.command()
@asyncio.coroutine
def what():
    yield from context.channel.send( "Hey, buddy. I'm Kenny. Type \"~hey\" or type \"~help\" to see what I can do." )
...
def wr( context, *, message ):
    try:
        ...
        if( len( message_split ) == 2 ):
            yield from context.channel.send( league_api.get_winrate( summoner_name, champion, "", "" ) )
        elif( len( message_split ) == 3 ):
            queue = message_split[2]
            yield from context.channel.send( league_api.get_winrate( summoner_name, champion, queue, "") )
        else:
            queue = message_split[2]
            season = message_split[3]
            yield from context.channel.send( league_api.get_winrate( summoner_name, champion, queue, season) )
    except:
        yield from context.channel.send( "Yeah, you messed that one up, buddy." )
...
def rwr( context, *, message ):
    try:
        ...
        yield from context.channel.send( league_api.get_current_ranked_winrate( summoner_name, champion ) )
    except:
        yield from context.channel.send( "Yeah, you messed that one up, buddy." )
